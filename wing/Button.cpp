...
namespace cwing {
	Button::Button(int x, int y, int w, int h, std::string txt): Component(x,y,w,h){
		...
		upIcon = IMG_LoadTexture(sys.get_ren(), "C:/Users/Johan.Eklundh/Desktop/Visual Studio/workspace/prog3_GEproj/images and sounds/Uppknapp.png");		//vänd \ ??
	...
	void Button::draw() const {
		if (isDown) {
			SDL_RenderCopy(sys.get_ren(), downIcon, NULL, &getRect());		// röd bakgrund
		}
		else {
